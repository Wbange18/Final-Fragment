--Get Roblox Services
local TweenService = _G.RobloxServices.TweenService
...
function Tools:LockPlayer()
	...
	repeat local success = pcall(function() game:FindService("StarterGui"):SetCore("ResetButtonCallback", false) end) wait(0.2) until success
end
...
function Tools:CreateRemote(name, Parent)
	...
	local Parent = Parent or ReplicatedStorage
...
function Tools:WaitForEvent(Remote, key)
	local playerInput, keyInput, resultInput
	if RunService:IsClient() then
		repeat
	...
	if RunService:IsServer() then
		repeat
			playerInput, keyInput, resultInput = Remote.OnClientEvent:Wait()
		until keyInput == key
		return resultInput
	end
end
