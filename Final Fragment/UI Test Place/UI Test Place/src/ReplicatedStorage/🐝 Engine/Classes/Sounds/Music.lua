	...
	coroutine.wrap(function()
		while self.Instance.Playing do
			...
			steppedConnection = RunService.Stepped:Connect(function()
				returnFactor = Engine.Tools:Flip(Engine.Tools:BoolToNumber(self.Instance.Playing))
...
function Music:Fade(direction, fadeTime)
	...
	if direction == "In" then
		FadeTween = Engine.Tools:QuickTween(self.Instance, fadeTime, {Volume = self.Volume}, "Linear")
	end
	if direction == "Out" then
		FadeTween = Engine.Tools:QuickTween(self.Instance, fadeTime, {Volume = 0}, "Linear")
	end
end
